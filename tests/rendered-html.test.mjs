...
test("includes personal headings and professional reports", async () => {
  ...
  assert.match(dashboard, /savingsGradient/);
  assert.match(dashboard, /Saved is calculated automatically/);
...
test("migrates legacy phone data and supports offline launches", async () => {
  ...
  assert.match(serviceWorker, /pocket-offline-v12/);
